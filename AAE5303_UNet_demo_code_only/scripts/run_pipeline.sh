...
SUBMISSION_JSON="output/AMtown02_submission_auto.json"
FIGURES_DIR="figures"
PROJECT_REPORT_MD="docs/PROJECT_REPORT.md"
TEAM_NAME="YourTeam"
REPO_URL="https://github.com/xxxxxx.git"
TRAIN_IMGS_DIR="data/amtown_train_imgs"
TRAIN_MASKS_DIR="data/amtown_train_masks"
TEST_IMGS_DIR="data/amtown_test_imgs"
TEST_MASKS_DIR="data/amtown_test_masks"
TRAIN_SPLIT_FILE="data/amtown_train.txt"
VAL_SPLIT_FILE="data/amtown_val.txt"
TEST_SPLIT_FILE="data/amtown_test.txt"
...
注意：
  ...
  3) 数据划分默认：
       - 训练池：AMtown01 + AMtown03（其中 15% 作为 val）
       - 测试集：AMtown02
EOF
}

# 准备 AMtown 数据集与 split 文件
prepare_amtown_data() {
  if [[ ! -f "scripts/prepare_amtown_dataset.py" ]]; then
    echo "[错误] 未找到 scripts/prepare_amtown_dataset.py"
    exit 1
  fi
  echo "== 准备 AMtown 数据（01+03 训练，02 测试） =="
  python scripts/prepare_amtown_dataset.py --project-root "$ROOT_DIR" --val-ratio 0.15 --seed 42

  for f in "${TRAIN_SPLIT_FILE}" "${VAL_SPLIT_FILE}" "${TEST_SPLIT_FILE}"; do
    if [[ ! -s "$f" ]]; then
      echo "[错误] split 文件缺失或为空: $f"
      exit 1
    fi
  done
}
...
run_train() {
  ...
  prepare_amtown_data
  echo "== 开始训练 =="
  echo "命令: python train.py --img-dir ${TRAIN_IMGS_DIR} --mask-dir ${TRAIN_MASKS_DIR} --epochs ${EPOCHS} --batch-size ${BATCH_SIZE} --learning-rate ${LR} --scale ${SCALE} --validation ${VAL_PERCENT} --classes ${CLASSES} --device ${DEVICE} --amp --stable-mode --num-workers -1 --optimizer adamw --scheduler cosine --loss-name ce_dice --weighted-loss --augmentation --weighted-sampler $*"
  python train.py \
    --img-dir "${TRAIN_IMGS_DIR}" \
    --mask-dir "${TRAIN_MASKS_DIR}" \
    --epochs "${EPOCHS}" \
...
run_eval() {
  ...
  prepare_amtown_data
  echo "== 开始评估 =="
  echo "命令: python evaluate_submission.py --model ${MODEL_PATH} --imgs-dir ${TEST_IMGS_DIR} --masks-dir ${TEST_MASKS_DIR} --split-file ${TEST_SPLIT_FILE} --scale ${SCALE} --output ${OUTPUT_JSON} --team ${TEAM_NAME} --repo-url ${REPO_URL} --device ${DEVICE} $*"
  python evaluate_submission.py \
    --model "${MODEL_PATH}" \
    --imgs-dir "${TEST_IMGS_DIR}" \
    --masks-dir "${TEST_MASKS_DIR}" \
    --split-file "${TEST_SPLIT_FILE}" \
    --scale "${SCALE}" \
    --output "${OUTPUT_JSON}" \
    --team "${TEAM_NAME}" \
    --repo-url "${REPO_URL}" \
    ...
    --val-split-file "${VAL_SPLIT_FILE}" \
    "$@"

  # 自动生成 leaderboard 提交格式 JSON
  echo "== 生成提交 JSON =="
  python - <<PY
import json
