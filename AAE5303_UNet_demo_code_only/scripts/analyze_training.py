...
def plot_class_distribution(class_info, output_path: str):
    ...
    # Add statistics text (guard against zero-frequency classes)
    non_zero_freq = [f for f in frequencies if f > 0]
    if non_zero_freq:
        ratio_text = f'{max(frequencies)/min(non_zero_freq):.0f}:1'
    else:
        ratio_text = 'N/A'
    stats_text = f'Class Imbalance Analysis:\n'
    stats_text += f'• Max: {max(frequencies):.2f}%\n'
    stats_text += f'• Min: {min(frequencies):.2f}%\n'
    stats_text += f'• Ratio: {ratio_text}'
